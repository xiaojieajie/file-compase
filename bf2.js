...
const question = ({ tipsMsg, type = 'input'}) => {
 ...
 * @returns 用户输入信息
 */
async function askingHandler({ tipsMsg, errMsg, condition }) {
  // 如果用户输入的正确，返回用户输入，否贼递归
  const { input: answer } = await question({ tipsMsg })
  if (condition(answer)) {
    return answer
  }
  chalkConsole({ color: 'red', str: errMsg }) // 提示
  return await askingHandler({ tipsMsg, errMsg, condition })
}
...
async function init() {
  try {
    const userPath = await getUserQuestion() // 获取用户输入
    if (!userPath) {
      return
    }
...
async function comPressThePicture(assetsPath) {
  ...
  try {
    ...
    await getUserQuestion() // 重新询问用户
    return
  }

  chalkConsole({ str: `共有${result.length}张图片` })
  
  const ask = await askingHandler({ tipsMsg: '是否要进行压缩，输入1开始压缩：', errMsg: '请输入1开启压缩，或使用ctrl+c ｜ exit退出程序', condition: answer => Number(answer) === 1 })
  if (!ask) return
